 ...
 * jmemdatasrc.c
 *
 * Created 2011 by Tom Atwood from jdatasrc.c, which is:
 *
 ...
 * source manager for this purpose.
 */
...
fill_input_buffer( j_decompress_ptr cinfo )
{
	...
	if ( pSourceManager -> start_of_file )
		nbytes = pSourceManager -> nBytesInSourceBuffer;	// The buffer is big enough to hold the entire image.
	else
		nbytes = 0;											// Nothing left beyond the entire image.
 ...
 * The caller must have already allocated and filled the buffer, and is responsible
...
jpeg_memory_src( j_decompress_ptr cinfo, char *pSourceBuffer, unsigned long nBytesInSourceBuffer )
{
	...
	* of JPEG images can be read from the same buffer by calling jpeg_stdio_src
